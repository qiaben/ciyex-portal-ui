...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/fhir/insurance/my`, {
      method: 'GET',
      headers: hdrs,
    });
    ...
    console.error('Portal insurance proxy error:', error);
    return NextResponse.json(
      { success: false, message: 'Insurance service unavailable' },
      { status: 500 }
    );
  }
}
