...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/fhir/portal/notifications/my`, {
      method: 'GET',
      headers: hdrs,
    });
    ...
    console.error('Portal notifications proxy error:', error);
    return NextResponse.json(
      { success: false, message: 'Failed to fetch notifications', data: [] },
      { status: 500 }
    );
  }
}
