...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ coverageId: string }> }) {
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/portal/insurance/${coverageId}`, { method: 'PUT', headers: getHeaders(request), body: JSON.stringify(body) });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch (error) {
    console.error('Portal insurance PUT error:', error);
    return NextResponse.json({ success: false, message: 'Failed to update insurance' }, { status: 500 });
  }
}
